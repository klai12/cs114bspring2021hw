...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        ### TODO:
        ###     1) Create a parameter matrix (weights and bias) for the hidden layer
        ###        using `torch.empty` or similar.
        ###        Initialize parameters with the `nn.init.xavier_uniform_` function
        ###        with default parameters.
        ###        Declare `self.embed_to_hidden_weight` as `nn.Parameter` with this as its data.
        ###     2) Create a parameter matrix (weights and bias) for the output layer
        ###        using `torch.empty` or similar.
        ###        Initialize parameters with the `nn.init.xavier_uniform_` function
        ###        with default parameters.
